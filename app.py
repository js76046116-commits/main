...
class SimpleHybridRetriever:
    ...
    def invoke(self, query):
        ...
        for doc in (docs_c1 + docs_c2):
            try:
                idx = int(doc.page_content) # 번호표 확인
                original_item = self.raw_data[idx] # 원본 데이터 조회
...
def load_search_system():
    ...
    if not os.path.exists(DB_PATH_1) or not os.path.exists(DB_PATH_2):
        st.error("❌ 분할된 DB 폴더(part1, part2)가 없습니다.")
        st.stop()
    ...
    reranker = CrossEncoder("BAAI/bge-reranker-v2-m3", model_kwargs={"torch_dtype": "auto"})
...
# ==========================================================
# [2] RAG 로직 (사장님이 좋아하신 v4.0 로직 그대로)
# ==========================================================
        ...
        with st.expander("🤔 답변이 부족한가요? (여기를 눌러 심층 검색)"):
            ...
            if st.button("🚀 HyDE 심층 검색 실행"):
                with st.spinner("🧠 전문가 키워드 추출 및 재검색 중..."):
                    ...
                    st.success(f"확장된 검색어: {final_query}")
                    st.markdown("---")
